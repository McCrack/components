...
<template>
    <table class="full-width font-13" cellpadding="0" cellspacing="0" rules="cols" bordercolor="#CCC">
        ...
        <thead class="h-32">
        <tr v-if="keyName || valueName" v-bind:class="captionClass || 'primary-bg white-txt'">
            <td></td>
            <th>{{keyName}}</th>
            <th>{{valueName}}</th>
            <td colspan="2"></td>
        ...
        <tr v-for="([key, value], index) of model" v-bind:key="index">
            <th title="Insert row"
                class="white-bg black-txt hover-primary-bg"
                v-on:click="insertRow(index + 1)">+</th>
            <td>
                <input v-model="model[index][0]" class="full-width transparent border-none font-13">
            </td>
            <td>
                <input v-model="model[index][1]" class="full-width transparent border-none font-13">
            </td>
            <th title="Swap row"
